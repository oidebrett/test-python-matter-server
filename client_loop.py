    ...
    async with session.ws_connect(URL) as ws:
        ...
        while(True):
            ...
            async for msg in ws:
                ...
                await pollForCommand(_sample_file_name , ws)

                if msg.type in (aiohttp.WSMsgType.TEXT, aiohttp.WSMsgType.BINARY):
                    message_results.append(json.loads(msg.data))
                    break
...
async def pollForCommand(file_name: str, ws):
    messageObject = loadAndCleanSampleData(file_name)
    print(messageObject)
